   ...
   public function excel_fileput($filePath1,$data,$tablename)
   {
	   ...
	   for($currentRow = 2;$currentRow<=$allRow;$currentRow++)
	   {
		   ...
		   {   
			   ...
			   {
				   $variable = $currentSheet->getCellByColumnAndRow(ord($currentColumn)-65,$currentRow)->getValue();
                   $val = $this->Origin_model->get_row($variable);				   
				   
			   }
			   else if($currentColumn=='D')
			   {
				   $value = $currentSheet->getCellByColumnAndRow(ord($currentColumn)-65,$currentRow)->getValue();
                   $val = $this->Port_model->get_row($value);				   
                   				   
			   }
			    else if($currentColumn=='I')
			   {
				   $values = $currentSheet->getCellByColumnAndRow(ord($currentColumn)-65,$currentRow)->getValue();
                   $val = $this->Seller_model->get_row($values);				   
	 ...
	 foreach ($total as $key => $value)
	  {
		  ...
		  $PHPExcel->setActiveSheetIndex(0)
				   ...
				   ->setCellValue('B'.$num,$value['TDC_URL']);
				   //->setCellValue('D'.$num,$value['TDC_TYPE']);
	  }
	  $PHPExcel->getActiveSheet()->setTitle('二维码号段信息导出-'.date('Y-m-d',time()));
	  $PHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(75);
	  $PHPExcel->getActiveSheet()->getStyle('A1:B1000')->getAlignment()->setHorizontal(\PHPExcel_Style_Alignment::HORIZONTAL_CENTER);
